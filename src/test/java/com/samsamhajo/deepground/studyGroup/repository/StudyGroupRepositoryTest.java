...
class StudyGroupRepositoryTest extends IntegrationTestSupport {
  ...
  void setUp() {
    ...
    IntStream.rangeClosed(1, 20).forEach(i -> {
      ...
      StudyGroup groupForSearch = StudyGroup.of(
          null,
          title,
          explanation,
          LocalDate.now().plusDays(1),
      ...
      groupForSearch.changeGroupStatus(status);
      studyGroupRepository.save(groupForSearch);
    });
  }
  ...
  void findWithMemberAndCommentsById() {
    Optional<StudyGroup> result = studyGroupRepository.findWithCreatorAndCommentsById(savedGroup.getId());

    assertThat(result).isPresent();
    StudyGroup group = result.get();

    assertThat(group.getTitle()).isEqualTo("테스트 스터디");
    assertThat(group.getCreator()).isNotNull(); // 작성자
    assertThat(group.getMembers()).hasSize(1); // 그룹 멤버 1명
    assertThat(group.getComments()).hasSize(1); // 댓글 1개
  }

  // TODO: 레포지토리 검색 메서드가 searchWithFilters로 통합됨에 따라 테스트 코드 수정 필요
  // @Test
  ...
  //
  //   assertThat(page.getTotalElements()).isEqualTo(21); // 1(테스트 스터디) + 20(검색용)
